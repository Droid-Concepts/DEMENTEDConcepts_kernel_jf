...
export INITRAMFS_SOURCE=`readlink -f ..`/jf/Ramdisks/AOSP_TMO
...
make VARIANT_DEFCONFIG=jf_tmo_defconfig SELINUX_DEFCONFIG=jfselinux_defconfig SELINUX_LOG_DEFCONFIG=jfselinux_log_defconfig DEMENTED_flashable_defconfig

HOST_CHECK=`uname -n`
if [ $HOST_CHECK = 'cphelps76-HP-Pavilion-dv7' ] || [ $HOST_CHECK = 'davros64' ]; then
...
cp Packages/03dementedcpuclock $PACKAGEDIR/system/etc/init.d/03dementedcpuclock
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	rm ramdisk.gz
	rm zImage
	zip -r ../DEMENTEDKernel-JFTMO-$curdate.zip .
        cp $FACTORY/DEMENTEDKernel-JFTMO-$curdate.zip $kout/DEMENTEDKernel-JFTMO-$curdate.zip
	cd $KERNELDIR
else
